...
var i,
	...
	// Usado para iframes
	// Ver setDocument ()
	// La eliminación del contenedor de la función provoca un "Permiso denegado"
	// error en IE
...
setDocument = Sizzle.setDocument = function (nodo) {
	...
	support.scope = assert (función (el) {
...
var rootjQuery,
	...
	init = jQuery.fn.init = función (selector, contexto, raíz) {
		...
		// para que migrate admita jQuery.sub (gh-2101)
			...
			para (; i <l; i ++) {
				para (cur = this [i]; cur && cur! == context; cur = cur.parentNode) {
					...
					if (cur.nodeType <11 && (objetivos?
						...
						// No le pase elementos que no sean a Sizzle
						cur.nodeType === 1 &&
...
function adoptValue (valor, resolver, rechazar, noValor) {
	método var;
							...
							var that = this,
								...
								mightThrow = function () {
									...
									luego = devuelto &&
										...
										// Verifique solo objetos y funciones para determinar su capacidad
...
Data.prototype = {
	...
	eliminar: función (propietario, clave) {
		...
		if (clave === indefinido || jQuery.isEmptyObject (caché)) {
			...
			// de los nodos DOM, así que configúrelo en undefined en su lugar
...
jQuery.extend ({
	...
	dequeue: function (elem, type) {
		...
		si (fn) {
			...
			// automáticamente retirado de la cola
			if (escriba === "fx") {
...
function getAll (contexto, etiqueta) {
	...
	// Utilice typeof para evitar la invocación del método de argumento cero en objetos host (# 15151)
	var ret;
...
jQuery.event = {
	...
	remove: function (elem, types, handler, selector, mappedTypes) {
		...
		// Eliminar datos y expandir si ya no se usan
	...
	despacho: function (nativeEvent) {
		...
		while ((coincidente = handlerQueue [i ++]) &&! event.isPropagationStopped ()) {
			...
			while ((handleObj = match.handlers [j ++]) &&
				...
				// especialmente universal o sus espacios de nombres son un superconjunto de los eventos.
	...
	controladores: función (evento, controladores) {
		...
		si (delegateCount &&
			...
			// Soporte: IE <= 9
			// Árboles de instancia <use> de Black-Hole SVG (trac-13180)
			cur.nodeType &&
			...
			if ((event.isTrigger & 1) && this [type]) {
				...
				if (! Saved.length) {
					...
					si (guardado! == resultado) {
						...
						// dejando el `resultado` sin definir. Necesitamos protegernos contra esto.
...
function domManip (colección, argumentos, devolución de llamada, ignorado) {
...
function boxModelAdjustment (elem, dimension, box, isBorderBox, styles, computedVal) {
	...
	para (; i <4; i + = 2) {
		...
		if (! isBorderBox) {
			...
			// Pero aún mantén un registro de lo contrario
			} demás {
...
function defaultPrefilter (elem, props, opts) {
	...
	if (! opts.queue) {
		...
		if (hooks.unqueued == null) {
			hooks.unqueued = 0;
			fuego antiguo = hooks.empty.fire;
...
jQuery.each (jQuery.expr.match.bool.source.match (/ \ w + / g), function (_i, name) {
	...
	attrHandle [nombre] = función (elem, nombre, isXML) {
		...
		if (! isXML) {
			...
			attrHandle [nombre en minúscula] = ret;
...
jQuery.fn.extend ({
	...
	removeClass: function (value) {
		...
		if (isFunction (valor)) {
			devuelve this.each (function (j) {
				jQuery (esto) .removeClass (value.call (this, j, getClass (esto)));
			});
		}
	...
	toggleClass: function (value, stateVal) {
		...
		devuelve this.each (function () {
			...
			// Cambiar el nombre completo de la clase
				...
				// retroceder a la cadena vacía si no se almacenó nada.
			...
			get: function (elem) {
				...
				para (; i <max; i ++) {
					...
					if ((opción.seleccionada || i === índice) &&
							...
							// No devuelva opciones que estén deshabilitadas o en un optgroup deshabilitado
							! option.disabled &&
...
jQuery.extend (jQuery.event, {
	...
	disparador: función (evento, datos, elem, onlyHandlers) {
		...
		if (! onlyHandlers &&! event.isDefaultPrevented ()) {
			...
			if ((! special._default ||
				special._default.apply (eventPath.pop (), datos) === falso) &&
				acceptData (elem)) {
	...
	if (Array.isArray (obj)) {
		...
		jQuery.each (obj, function (i, v) {
			if (tradicional || rbracket.test (prefijo)) {
				...
				buildParams (
					prefijo + "[" + (typeof v === "objeto" && v! = null? i: "") + "]",
					v,
					tradicional,
					agregar
				);
...
function ajaxHandleResponses (s, jqXHR, respuestas) {
	...
	// Elimina el tipo de datos automático y obtén el tipo de contenido en el proceso
	while (dataTypes [0] === "*") {
		dataTypes.shift ();
		if (ct === undefined) {
		...
		si (actual) {
			...
			// Convierta la respuesta si prev dataType no es automático y difiere del actual
				...
				if (conv! == true) {
					...
					// A menos que se permita que aparezcan errores, captúrelos y devuélvalos
					if (conv && s.throws) {
...
jQuery.extend ({
	...
	ajaxSettings: {
		...
		* /
		...
		// Las claves separan la fuente (o catchall "*") y los tipos de destino con un solo espacio
		convertidores: {
	...
	ajax: function (url, opciones) {
		...
		transporte var,
			...
			jqXHR = {
				...
				// Anula el encabezado de tipo de contenido de la respuesta
		...
		if (! s.hasContent) {
			...
			// Si los datos están disponibles y deben procesarse, añádalos a la URL
			...
			s.url = cacheURL + sin almacenar en caché;
		...
		if (! transporte) {
			...
			tratar {
				...
				// Relanzar excepciones posteriores a la finalización
				if (completado) {
					lanzar e;
				}
		...
		función realizada (estado, nativeStatusText, respuestas, encabezados) {
			...
			// Encabezados de respuesta en caché
			...
			// Convierta sin importar qué (de esa manera los campos responseXXX siempre se establecen)
...
jQuery.ajaxTransport (función (opciones) {
	...
	if (support.cors || xhrSupported &&! options.crossDomain) {
		regreso {
			enviar: función (encabezados, completa) {
				var i,
					xhr = opciones.xhr ();
							...
							if (escriba === "abortar") {
								...
								if (typeof xhr.status! == "number") {
									...
									completo(
										...
										// Archivo: el protocolo siempre da el estado 0; ver # 8605, # 14207
										xhr.status,
										xhr.statusText
									);
								}
				...
				if (xhr.onabort! == indefinido) {
					...
					xhr.onreadystatechange = function () {
						...
						// Compruebe readyState antes del tiempo de espera a medida que cambia
						if (xhr.readyState === 4) {
...
jQuery.ajaxSetup ({
	acepta: {
		secuencia de comandos: "texto / javascript, aplicación / javascript," +
...
// Por eso, esta medida de seguridad debe desactivarse en Safari 8.
...
jQuery.parseHTML = función (datos, contexto, keepScripts) {
	...
	if (! context) {
		...
		// utilizando document.implementation
...
jQuery.each ({Alto: "alto", Ancho: "ancho"}, función (nombre, tipo) {
	jQuery.each ({
		...
		jQuery.fn [funcName] = función (margen, valor) {
			var chainable = argumentos.length && (defaultExtra || typeof margin! == "boolean"),
...
jQuery.each (
	("desenfoque enfoque focusin focusout redimensionar desplazarse hacer clic doble clic" +
	"mousedown mouseup mousemove mouseover mouseout mouseout mouseenter mouseleave" +
	"cambiar seleccionar enviar tecla pulsada tecla presionar tecla menú contextual") .split (""),
	función (_i, nombre) {
